...
export function useReaderChapterContext(
  options: UseReaderChapterContextOptions,
) {
  const localAddedShelfId = ref("");
  const currentShelfId = computed(
    () => readSource(options.shelfBookId) ?? localAddedShelfId.value,
  );
  ...
  function getChapter(index: number): ChapterItem | undefined {
    return index >= 0 && index < chapters.value.length
      ? chapters.value[index]
      : undefined;
  }

  const activeChapter = computed(() =>
    getChapter(options.activeChapterIndex.value),
  );
  const hasPrev = computed(() => options.activeChapterIndex.value > 0);
  const hasNext = computed(
    () => options.activeChapterIndex.value < chapters.value.length - 1,
  );
  const currentChapterName = computed(
    () => activeChapter.value?.name ?? fallbackChapterName.value,
  );
  const currentChapterUrl = computed(
    () => activeChapter.value?.url ?? fallbackChapterUrl.value,
  );
  ...
  const activeSkinLockedFlipMode = computed<string | null>(() => {
    ...
    return (
      options.readerSkins.value.find((skin) => skin.localId === skinId)
        ?.lockedFlipMode ?? null
    );
  });
  ...
  const isScrollMode = computed(
    () =>
      !isComicMode.value &&
      !isVideoMode.value &&
      effectiveFlipMode.value === "scroll",
  );
