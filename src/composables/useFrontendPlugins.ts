...
import { resolveBackgroundDefinitionSources } from "@/features/frontendPlugins/pluginNormalizer";
import {
...
async function refreshRuntimeBackgroundDefinitions(
  records: RuntimePluginRecord[] = runtimePlugins,
): Promise<void> {
  ...
  for (const record of records) {
    ...
    const context: PluginSettingsContext = {
      values: cloneValue(readPluginSettings(record)),
      session: currentReaderSession,
      meta: readonly(record) as Readonly<FrontendPluginRecord>,
      api,
    };
    try {
      record.backgrounds = await resolveBackgroundDefinitionSources(
        record,
        record.backgroundSources,
        context,
      );
    } catch (error) {
      record.status = "error";
...
async function syncPublicBackgroundState(): Promise<void> {
  await refreshRuntimeBackgroundDefinitions();
