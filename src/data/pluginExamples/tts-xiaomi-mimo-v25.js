...
legado.registerPlugin({
  ...
  setup: function (api) {
    ...
    function ensureApiKey() {
      ...
      if (!apiKey) {
        throw new Error(
          "请先在「小米 MiMo V2.5 TTS 引擎」插件设置中填写 API Key",
        );
      }
      return apiKey;
    }
    ...
    function buildAudioOptions(context, model) {
      ...
      if (model === "mimo-v2.5-tts") {
        ...
        audio.optimize_text_preview = settingBoolean(
          "optimizeTextPreview",
          false,
        );
    ...
    function parseAudioData(body) {
      ...
      if (data && data.error) {
        throw new Error(
          data.error.message || data.error.code || "MiMo TTS 请求失败",
        );
      }
    ...
    return {
      settings: {
        defaults: {
          ...
          voice: "mimo_default",
          stylePrompt:
            "自然、清晰、适合长篇小说朗读。根据文本内容保留情绪，不额外解释。",
          voiceDesignPrompt: "",
          voiceCloneData: "",
        ...
        schema: function (context) {
          ...
          return [
            ...
            },
            {
              ...
              placeholder:
                "data:audio/mpeg;base64,... 或 data:audio/wav;base64,...",
            },
            {
              type: "number",
              key: "timeoutSecs",
